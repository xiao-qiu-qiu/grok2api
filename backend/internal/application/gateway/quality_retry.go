...
func qualityHasSustainedCipherStream(sig QualityStreamSignals) bool {
	return sig.HasThinking && !sig.HasReasoningDelta && sig.FirstVisible &&
		sig.VisibleTokens >= defaultBurstMaxVisible &&
...
// Remaining plaintext deltas still deliver. Cipher-only HasThinking is
// released only after substantial visible output has streamed continuously;
// a delayed one-shot fake-encrypted dump has no sustained visible span.
func ClassifyQualityHold(sig QualityStreamSignals, minOutput int64) QualityVerdict {
	if minOutput <= 0 {
		minOutput = defaultQualityMinOutput
	}
	if qualityIsCipherDrool(sig, minOutput) {
		return QualityWithhold
	}
	dump := qualityIsBurstDump(sig, minOutput) ||
		qualityIsFakeEncryptedDump(sig, minOutput) ||
		qualityIsFastReasoningRatioDump(sig)
	if dump && !qualityHasSustainedCipherStream(sig) {
	...
	if sig.HasThinking {
		...
		// Cipher-only: do not release when encrypted_content first meets the
		// floor. Require strong ciphertext plus sustained visible streaming so
		// a delayed one-shot fake-encrypted dump cannot bypass the detectors.
		if sig.Terminal {
			return QualityDeliver
		}
		...
		if qualityHasSustainedCipherStream(sig) {
			return QualityDeliver
		}
